...
try:
    ...
    candedat = []
    for cok in cookies:
        browser.add_cookie(cok)
        browser.refresh()

        time.sleep(1)
        age = browser.find_element(By.ID, 'info').find_element(By.ID, 'age').text.split(' ')[-1]
        skills = len([i.text for i in browser.find_element(By.ID, 'skillsList')
                     .find_elements(By.TAG_NAME, 'li')])
        browser.delete_all_cookies()
        candedat.append({'cookie': cok['value'], 'age': age, 'skills': skills})

    sorted_candedat = sorted(candedat, key=lambda x: (x['age'], -x['skills']))
    print(sorted_candedat[0]['cookie'])
